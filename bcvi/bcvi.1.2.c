...
/* This is version 1.2,    for exploits due 9/29/2017. */

long bcvi_version = 120; /* 1.2 */
...
int run_macro(void) {
	...
	if (b == 0xeb || b == 0xe9 || b == 0xe8) {
	    ...
	    illegal = b;
	    break;
	} else if (b == 0xcd || b == 0x34 || b == 0x05) {
	    /* forbid int $0x80, sysenter, and syscall */
	    illegal = b;
	    break;
	}
    }
...
void ex_mode(void) {
    ...
    echo();
    mvgetnstr(WINDOW_HEIGHT, 1, cmd_buf, COLS);
    noecho();
