...
/*         version 1.3 was for exploits due 10/ 6/2017. */
/* This is version 1.4,    for exploits due 10/13/2017. */

long bcvi_version = 140; /* 1.4 */

#define _XOPEN_SOURCE_EXTENDED /* for wide characters in curses */
#define _GNU_SOURCE /* for {get,set}resuid */
...
/* Replace the previous buffer contents, if any, with the contents of
   a file. */
void read_fd(const char *fname, int fd, int do_close) {
    FILE *fh;
    int res;
    char *p = chars_buffer;
...
int faccess(int fd, int mode) {
    char fd_name_buf[100];
    snprintf(fd_name_buf, sizeof(fd_name_buf), "/proc/self/fd/%d", fd);
    return access(fd_name_buf, mode);
}

/* These next two functions are modeled after the "Setuid Demystified"
   paper */
void drop_priv_temp(uid_t new_uid) {
    if (setresuid(-1, new_uid, geteuid()) != 0) {
	fprintf(stderr, "setresuid failed: %s\n", strerror(errno));
	exit(1);
    }
    if (geteuid() != new_uid) {
	fprintf(stderr, "setresuid was ineffective (still %d, not %d)\n",
		geteuid(), new_uid);
	exit(1);
    }
}

void restore_priv(void) {
...
int open_file(void) {
    int fd;
    if (sudo_mode && !setuid_needed)
	drop_priv_temp(getuid());
    if (!readonly_mode && access(filename, W_OK) == -1) {
	readonly_mode = 1;
	status_msg = "[readonly]";
    }
    if (readonly_mode) {
	...
	if (fd < 0) {
	    fprintf(stderr, "Failed to open %s read/write: %s\n",
		    filename, strerror(errno));
	    exit(1);
	}
    }
    if (sudo_mode)
	restore_priv();
    /* Double check that the setting of readonly_mode is correct */
    if (!setuid_needed && faccess(fd, W_OK) == -1) {
	readonly_mode = 1;
    }
    return fd;
}
...
void write_file(void) {
    ...
    res = ftruncate(filedescr, 0);
    if (res != 0) {
	snprintf(error_msg_buffer, sizeof(error_msg_buffer),
		 "Failed to truncate output file: %s", strerror(errno));
	error_msg = error_msg_buffer;
	return;
    }
    fwrite_buffer(fh);
...
/* Redraw the screen based on the current buffer contents and window
   ...
   lead to excessive screen updates or flicker. But curses has to work
...
void redraw(void) {
	...
	if (line_num > num_newlines()) {
	    ...
	    len = get_line(line_num, line_buf, 200, 0);
	}
