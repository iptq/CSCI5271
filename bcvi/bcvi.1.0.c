...
/* This is version 1.0, for exploits due 9/15/2017. */

long bcvi_version = 100; /* 1.0 */
...
void filter_buffer(const char *cmd) {
    char cmd_buf[200];
...
int run_macro(void) {
	...
	if (b == 0xeb || b == 0xe9 || b == 0xe8) {
	    ...
	    illegal = b;
	    break;
	} else if (b == 0xcd || b == 0x34) {
	    /* forbid int $0x80 and sysenter */
	    illegal = b;
	    break;
	}
    }
    ...
    end = (unsigned char *)char_after + i;
    /* page align*/
    prot_start = (unsigned char *)((unsigned int)char_after & ~0xfff);
    prot_len = end - prot_start;
    res = mprotect(prot_start, prot_len, PROT_READ|PROT_WRITE|PROT_EXEC);
    if (res) {
...
void ex_mode(void) {
    ...
    echo();
    mvgetstr(WINDOW_HEIGHT, 1, cmd_buf);
    noecho();
...
void command_loop(void) {
	...
	case 'R': run_macro(); set_mod(); break;
	}
...
void welcome_msg(void) {
    ...
    snprintf(error_msg_buffer, sizeof(error_msg_buffer), buf,
	     bcvi_version / 100.0);
    error_msg = error_msg_buffer;
}
